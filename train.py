...
def vocab_func(vocab_path):
    ...
    id2item = {i+1:v for i,v in enumerate(vocab)}
...
class data_generator(DataGenerator):
    ...
    """
    def __iter__(self, random=False):
        batch_y, batch_contexts = [], []
        for is_end, txt in self.sample(random):
            #text = open(txt, encoding='utf-8').read()
            text = txt.strip()
            target,context = text.split("|")

            batch_y.append(int(target))

            batch_contexts.append([char2id.get(c,1) for c in context.strip()])

            if len(batch_y) == self.batch_size or is_end:
                batch_contexts = sequence_padding(batch_contexts)

                batch_y = np.array(batch_y)
                yield batch_contexts, batch_y
                batch_contexts, batch_y = [], []
...
def train():
    ...
    x = Embedding(len(id2char) + 2, embed_size, mask_zero=True)(input)
    ...
    checkpoint_cb = ModelCheckpoint(check_file_path,save_best_only=True)
    early_stopping_cb = EarlyStopping(patience=5,restore_best_weights=True)
    tensorboard_cb = TensorBoard(run_logdir)
    # tensorboard --logdir=./my_logs --port=6006

    model.compile(loss='sparse_categorical_crossentropy', optimizer=Adam(lr=lr, beta_1=0.9, beta_2=0.999),
                metrics=["accuracy"])
    ...
    dev_genertor = data_generator(dev_data)

    #     model.save('my_keras_model.h5')
    #     test_data = input_data_func(test_path)
    #     print('evaluate begins!!!!!!!!!!!!!!!!!!!')
    # y_pred = model.predict(dev_genertor)
    # print(y_pred)


    model.fit_generator(train_generator.forfit(), epochs=n_epochs,steps_per_epoch=len(train_generator),
                    validation_data=dev_genertor.forfit(),
                    validation_steps=len(dev_genertor),
