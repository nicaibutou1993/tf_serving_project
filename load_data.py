...
def build_dataset(path,shuffle=True):
    ...
    dataset = dataset.map(
        lambda line: (tf.strings.split(line,'|')[1], tf.strings.split(line,'|')[0]),
    ...
    dataset = dataset.padded_batch(batch_size,padded_shapes=([None],[]),padding_values=(tf.constant(pad_value, dtype=tf.string),[]))
    return dataset
